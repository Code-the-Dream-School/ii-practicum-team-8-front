...
const deleteBooking = async (bookingId, token) => {
  const res = await deleteData(`${endpointCalendar}/${bookingId}`, {}, getAuthHeaders(token));
  return res||{};
};

const updateBooking = async (booking, token) => {
  const { bookingId, ...requestBody } = booking;
  const res = await updateData(`${endpointCalendar}/${bookingId}`, requestBody, {}, getAuthHeaders(token));
  return res||{};
};

const createBooking = async (booking, token) => {
  const res = await postData(endpointCalendar, booking, {}, getAuthHeaders(token));
  return res||{};
};
